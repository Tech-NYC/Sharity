            ...
            mergedArray.map((data) => (
              <>
                ...
                <Box key={data.id} flexDirection="column" item xs={4} direction="column">
                  <Typography variant="h3" style={{ margin: "1%" }}>
                    {data.name}
                  </Typography>
                ...
                <Box flexDirection="column" item xs={4} direction="column" style={{ paddingRight: "5%" }}>
                  {userLoggedIn ? (
                    ...
                    <Button align="center" component={Link} to="/login" variant="outlined" style={{ margin: "auto" }}>
                      Request Pickup
                    </Button>
                  )}
                </Box>
      ...
      {orgNeeds.length === 0 ? (
        ...
        orgNeeds.map((data) => (
          <Box container display="flex" justify="center" align="center" style={{ width: "1200px", overflow: "hidden", margin: "auto", padding: "25px" }}>
            <Box style={{ width: "400px", float: "center" }}>
              <Typography variant="h6">Items Needed List</Typography>
              {!data.items_needed ? <Typography> No Items Needed Currently</Typography> : data.items_needed.split(",").map((item) => <Typography variant="subtitle1">{item}</Typography>)}
            </Box>
            <Box style={{ width: "400px", float: "center" }}>
              <Typography variant="h6">Item Approved Condition List </Typography>
              {!data.conditions_accepted ? <Typography>No Items Needed Currently</Typography> : data.conditions_accepted.split(",").map((item) => <Typography variant="subtitle1">{item}</Typography>)}
            </Box>
            <Box style={{ width: "400px", float: "center" }}>
              <Typography variant="h6">Item Not Approved Condition List </Typography>
              {!data.conditions_not_accepted ? (
                ...
                data.conditions_not_accepted.split(",").map((item) => <Typography variant="subtitle1">{item}</Typography>)
              )}
            </Box>
          </Box>
        ))
