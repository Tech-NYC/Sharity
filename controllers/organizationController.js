...
class organizationController {
  async create(request, response) {
    try {
      await db.none(
        "INSERT INTO organizations (first_name, last_name, username, email,phone_number, password_hash, is_organization) VALUES (${first_name},${last_name}, ${username}, ${email},${phone_number}, ${password_hash}, ${is_organization})",
        request.body
      );
  ...
  async fetch_requests(request, response) {
    try {
      ...
      const data = await db.any("SELECT * organizations WHERE user_id=$1 INNER JOIN ", user);
  ...
  async fetch_info() {
    try {
      const user = parseInt(request.params.id);
      const data = await db.any("SELECT * FROM organizations WHERE user_id=$1", user);
